...
`@tokens-as-parameters/dsh-formal-proof` 是一个可安装的 DeepSeek Harness Bundle。它在 DSH 官方 Code Agent 与 Agent Loop 之上增加由 Verifier 引导的多轨迹 Lean 证明搜索，不替代 Harness Runtime 或 Web UI。
...
Package Manifest 中的 `dsh.bundle.patch` 会组合五个插件：

- `observer`：持久化领域事件账本与 Run Snapshot；
- `roles`：作用域隔离的 Prover，以及只读 Reviewer 的 Prompt/Tool；
- `reflection`：具备受限证据工具的 Agentic 比较反思；
- `runtime`：后台 Run 生命周期、隔离 Worktree/Session、Checker 门控、语义合并与停止策略；
- `tools`：`proof_run_start`、`proof_run_status`、`proof_run_list` 和 `proof_run_stop`。
