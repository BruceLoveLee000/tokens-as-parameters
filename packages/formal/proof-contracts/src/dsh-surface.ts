...
export const ProofRunViewSchema = z.object({
  ...
  lanes: z.array(z.object({
    ...
    lossVerdict: z.enum(['invalid', 'no-progress', 'progress', 'solved']),
    candidateStatus: z.enum(['INVALID', 'EXPLORATORY', 'VERIFIED']),
  })),
...
export function toProofRunView(snapshot: ProofRunSnapshot): ProofRunView {
  return {
    ...
    lanes: snapshot.lanes.map(lane => ({
      ...
      lossVerdict: lane.loss.verdict,
      candidateStatus: lane.loss.candidateStatus,
    })),
