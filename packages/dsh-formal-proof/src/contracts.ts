...
export interface ReflectionRoute {
  rolloutId: string
  prompt: string
}
...
export interface ProofRunSnapshot {
  ...
  lanes: LaneEvidence[]
  latestReflection?: ReflectionPlan
...
export function neutralReflectionPlan(rolloutIds: readonly string[]): ReflectionPlan {
  return {
    reflection: 'No valid comparative update was submitted; preserve the verified baseline.',
    commonPrompt: 'Continue from the checker-verified baseline and use concrete Lean feedback.',
...
export function validateReflectionPlan(value: unknown, rolloutIds: readonly string[]): ReflectionPlan {
  ...
  if (new Set(actual).size !== actual.length || expected.join('\0') !== actual.join('\0')) {
    throw new Error(`reflection routes must cover exactly: ${expected.join(', ')}`)
  }
  return plan
}
