...
`@tokens-as-parameters/dsh-formal-proof` is one installable DeepSeek Harness Bundle. It augments the official DSH Code Agent and Agent Loop with verifier-guided, multi-trajectory Lean proof search. It does not replace the Harness runtime or Web UI.
...
The package manifest's `dsh.bundle.patch` composes five plugins:

- `observer`: durable domain-event ledger and run snapshots;
- `roles`: scoped Prover and read-only Reviewer prompts/tools;
- `reflection`: agentic comparative reflection with bounded evidence tools;
- `runtime`: background run lifecycle, isolated worktrees/sessions, checker gates, consolidation, and stop policy;
- `tools`: `proof_run_start`, `proof_run_status`, `proof_run_list`, and `proof_run_stop`.
